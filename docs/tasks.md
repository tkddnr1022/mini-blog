...
- [ ] `app/layout.tsx`에 공통 헤더/푸터 슬롯
- [ ] Navbar: 홈, 카테고리(또는 목록), 검색, About 링크
- [ ] 외부 링크(GitHub, 이메일) 자리 확보 (실데이터는 M5)
- **관련:** FR-12, DR-05, DR-06
- **완료 기준:** 모든 라우트에서 Navbar가 렌더된다

### T-M3-02. PostCard 컴포넌트
- [ ] 제목, 날짜, 카테고리, 읽는 시간, description, thumbnail 표시
- [ ] 포스트 상세로 링크
- **관련:** FR-06, DR-01
- **완료 기준:** 카드 클릭 시 `/posts/[slug]`로 이동한다

### T-M3-03. 홈 — 최신 포스트 목록
- [ ] `/`에서 date 기준 최신순 목록 렌더
- [ ] 빈 상태(포스트 0개) UI
- **관련:** PRD §6, §9.2
- **완료 기준:** 샘플 포스트가 홈에 노출된다

### T-M3-04. 포스트 상세 페이지
- [ ] `/posts/[slug]` SSG (`generateStaticParams`)
- [ ] 제목, 날짜, 카테고리, 읽는 시간, 본문 HTML 렌더
- [ ] 존재하지 않는 slug는 빌드에서 제외(정적 export 특성 준수)
- **관련:** FR-06, PRD §6
- **완료 기준:** 샘플 포스트 상세가 정적 HTML로 생성된다

### T-M3-05. 카테고리 목록 페이지
- [ ] `/category/[category]` SSG
- [ ] 해당 카테고리 포스트만 PostCard로 나열
- [ ] 홈/상세에서 카테고리 클릭 시 이동
- **관련:** FR-04
- **완료 기준:** 카테고리 URL로 필터된 목록이 표시된다

### T-M3-06. 이전/다음 글 네비게이션
- [ ] date 정렬 기준 인접 포스트 링크를 상세 하단에 표시
- [ ] 첫/마지막 글은 한쪽만 표시
- **관련:** FR-09
...
- [ ] 헤딩(h2/h3) 기반 목차 생성
- [ ] 앵커 클릭 시 해당 섹션으로 스크롤
- [ ] Intersection Observer(또는 동등)로 현재 섹션 하이라이트
- [ ] 데스크톱: 사이드, 모바일: 접이식/상단 등 반응형 배치
- **관련:** FR-07, DR-05
- **완료 기준:** 스크롤 시 현재 섹션이 하이라이트된다

### T-M3-08. 이미지 라이트박스
- [ ] `yet-another-react-lightbox` 연동
- [ ] 본문 이미지 클릭 시 확대, Esc/오버레이로 닫기
- [ ] 키보드 접근 가능
- **관련:** FR-08, NFR 접근성
