...
- [x] 시스템 설정 감지 + 수동 토글
- [x] FOUC 방지 (`class` 전략 + 인라인 스크립트 또는 next-themes)
- [x] giscus 테마 동기화
- **관련:** DR-03, FR-10
...
- [x] 모바일/태블릿/데스크톱에서 홈, 상세(TOC), 검색, About 레이아웃 확인·수정
- [x] 터치 타깃·가독성(본문 줄간격) 조정
- **관련:** DR-05
- **완료 기준:** 주요 뷰포트에서 레이아웃이 깨지지 않는다

### T-M6-03. 스크롤 반응형 Navbar

- [x] 스크롤 시 축소/블러 등 동적 변화 (절제된 범위)
- **관련:** DR-06
- **완료 기준:** 스크롤에 따라 Navbar 시각 상태가 변한다

### T-M6-04. Framer Motion 인터랙션

- [x] 페이지/목록 전환, 카드 hover 등 2~3곳 절제된 모션 적용
...
- [x] 시맨틱 헤딩/랜드마크, 키보드 포커스, 명도 대비 WCAG AA
- [x] 라이트박스·토글·검색 입력 포커스 트랩/복귀 점검
- **관련:** NFR 접근성
...
- [x] 불필요한 클라이언트 JS 최소화
- [x] 콘텐츠 이미지 해상도/포맷 가이드를 README에 한 절 추가
- [x] Lighthouse Performance 목표(90+) 로컬 측정
- **관련:** NFR 성능, 성공 지표
- **완료 기준:** 대표 페이지 Lighthouse Performance ≥ 90 (로컬 기준 기록)
- **로컬 측정:** 홈 `/` 100, 포스트 `/posts/2026-07-10-static-architecture` 100 (2026-07-14)
